...
class ControladorHotel():
    ...
    def remover(self):
        ...
        if not hotel_existe:
            raise Exception(f"Hotel de código [{codigo}] não foi encontrada para ser removida.")

    def listar(self):
        return map(lambda hotel : {"nome": hotel.nome, "endereço": hotel.endereco, "codigo": hotel.codigo}, self.__hoteis)
    ...
    def alterar(self):
        ...
        for hotel in self.__hoteis:
            if hotel.codigo == dados_hotel["codigo"]:
                ...
                self.__hoteis[hotel.codigo]["localizacao_hotel"] = dados_hotel["localizacao_hotel"]
        ...
        if not hotel_existe:
            raise Exception(f"Hotel de código [{dados_hotel["codigo"]}] não foi encontrada.")
