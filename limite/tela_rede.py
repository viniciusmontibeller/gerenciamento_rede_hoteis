...
class TelaRede(AbstractTela):
    ...
    def pega_dados_rede(self):
        ...
        layout = [
            ...
            [sg.Text('Nome:', size=(15, 1)), sg.InputText(key='nome', tooltip='Nome da rede')],
            [sg.Text('Código:', size=(15, 1)), sg.InputText(key='codigo', tooltip='Apenas números (ex: 123)')],
            [sg.Text('Localização da rede:', size=(15, 1)), sg.InputText(key='localizacao', tooltip='Informe a cidade')],
    ...
    def pega_dados_inclusao_de_hotel(self):
        sg.theme(self.ESTILO_JANELA)
        layout = [
            [sg.Text('DADOS HOTEL', font=("Helvica", 20), justification='center')],
            [sg.Text('Código da Rede:', size=(15, 1)), sg.InputText(key='codigo_rede', tooltip='Apenas números (ex: 123)')],
            [sg.Text('Código do Hotel:', size=(15, 1)), sg.InputText(key='codigo_hotel', tooltip='Apenas números (ex: 123)')],
            [sg.Button('Confirmar'), sg.Cancel('Cancelar')]
        ]
    ...
    def pega_codigo_rede(self):
        sg.theme(self.ESTILO_JANELA)
        layout = [
            [sg.Text('SELECIONAR REDE', font=("Helvica", 20), justification='center')],
            [sg.Text('Digite o código da rede escolhida', font=("Helvica", 15))],
            [sg.Text('Código:', size=(15, 1)), sg.InputText(key='codigo', tooltip='Digite apenas números (ex: 123)', size=(20, 1))],
            [sg.Button('Confirmar'), sg.Cancel('Cancelar')]
        ]
        self.__window = sg.Window(self.TITULO_BASE).Layout(layout)
        ...
        while True:
            ...
            if not self._validar_campos(values, [
                    ("obrigatorio", "codigo", "Código da Rede"),
                    ("numero", "codigo", "Código da Rede")
                ]):
                continue
